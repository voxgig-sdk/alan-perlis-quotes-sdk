# alan-perlis-quotes-mcp

[MCP](https://modelcontextprotocol.io) server exposing the AlanPerlisQuotes SDK as
two agent tools — `alan-perlis-quotes_list` and `alan-perlis-quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/alan-perlis-quotes-mcp)
make build

# 2. Provide credentials via the environment
export ALAN_PERLIS_QUOTES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user alan-perlis-quotes \
  -- /absolute/path/to/alan-perlis-quotes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./alan-perlis-quotes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// alan-perlis-quotes_list: first page of records
{ "entity": "quote" }
{ "entity": "quote", "query": { } }

// alan-perlis-quotes_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/alan-perlis-quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user alan-perlis-quotes \
     -- "$PWD"/dist/*/alan-perlis-quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `alan-perlis-quotes_list` and `alan-perlis-quotes_load` tools now appear
   in new sessions. Ask the agent to *"list quote using alan-perlis-quotes"*
   and it calls `alan-perlis-quotes_list` with `{"entity":"quote"}`.
...
```sh
export ALAN_PERLIS_QUOTES_APIKEY=sk_live_xxx            # API key
export ALAN_PERLIS_QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `alan-perlis-quotes_list` tool
...
### Call the `alan-perlis-quotes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `alan-perlis-quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `alan-perlis-quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ALAN_PERLIS_QUOTES_APIKEY` | API key sent with every request. |
| `ALAN_PERLIS_QUOTES_BASE` | Optional override of the API base URL. |
...
```sh
./alan-perlis-quotes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"alan-perlis-quotes_load","arguments":{"entity":"quote","query":{"id":1}}}}'
```
