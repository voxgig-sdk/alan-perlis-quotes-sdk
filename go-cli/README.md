# alan-perlis-quotes-cli

boru-driven command-line client **and** interactive REPL for the AlanPerlisQuotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/alan-perlis-quotes-cli)
make build

# 2. See usage (words, entities, env vars)
./alan-perlis-quotes-cli --help

# 3. Provide credentials once, via the environment
export ALAN_PERLIS_QUOTES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./alan-perlis-quotes-cli load 1 quote            # {id:1} shorthand
./alan-perlis-quotes-cli load '{id:1}' quote       # explicit match map

# 5. Override the API base URL for a single call
ALAN_PERLIS_QUOTES_BASE=https://api.example.com ./alan-perlis-quotes-cli load 1 quote

# 6. No arguments -> interactive REPL
./alan-perlis-quotes-cli
alan-perlis-quotes> load 1 quote
alan-perlis-quotes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/alan-perlis-quotes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/alan-perlis-quotes-cli load 1 quote
   ```
...
```sh
./alan-perlis-quotes-cli load 1 quote          # scalar shorthand for {id:1}
./alan-perlis-quotes-cli load '{id:1}' quote     # explicit match map
```
...
./alan-perlis-quotes-cli load 1 quote
```
...
Run with no arguments to open a REPL (prompt `alan-perlis-quotes>`). Each line is
evaluated as its own boru expression:

```text
$ ./alan-perlis-quotes-cli
alan-perlis-quotes> load 1 quote
alan-perlis-quotes> /help
alan-perlis-quotes> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/alan-perlis-quotes-cli`. |
...
(`./alan-perlis-quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
