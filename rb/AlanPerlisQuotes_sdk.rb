...
require_relative 'AlanPerlisQuotes_types'
...
class AlanPerlisQuotesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AlanPerlisQuotesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AlanPerlisQuotesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AlanPerlisQuotesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AlanPerlisQuotesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AlanPerlisQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AlanPerlisQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AlanPerlisQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => AlanPerlisQuotesError.new(
        "#{op}_allow",
        "AlanPerlisQuotesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AlanPerlisQuotesError => err
    ...
    if fetched.is_a?(Hash)
      status = AlanPerlisQuotesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AlanPerlisQuotesError.new(
        "graphql_error", "AlanPerlisQuotesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AlanPerlisQuotesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
