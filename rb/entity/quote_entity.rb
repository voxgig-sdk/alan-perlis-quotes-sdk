# AlanPerlisQuotes SDK Quote entity
...
class QuoteEntity
  ...
  def data_set(args)
    if args
      @_data = AlanPerlisQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = AlanPerlisQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Quote, Hash] the loaded Quote; raises AlanPerlisQuotesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AlanPerlisQuotesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
