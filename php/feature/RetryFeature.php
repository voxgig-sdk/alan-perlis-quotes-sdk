...
// AlanPerlisQuotes SDK retry feature
...
class AlanPerlisQuotesRetryFeature extends AlanPerlisQuotesBaseFeature
{
    ...
    public function init(AlanPerlisQuotesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AlanPerlisQuotesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AlanPerlisQuotesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AlanPerlisQuotesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
