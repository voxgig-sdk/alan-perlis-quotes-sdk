...
// AlanPerlisQuotes SDK pipeline test
//
...
{
    private static function utility(): AlanPerlisQuotesUtility
    {
        return new AlanPerlisQuotesUtility();
    }
    ...
    private static function ctx(array $over = []): AlanPerlisQuotesContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AlanPerlisQuotesContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AlanPerlisQuotesOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AlanPerlisQuotesError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AlanPerlisQuotesOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AlanPerlisQuotesMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AlanPerlisQuotesOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AlanPerlisQuotesMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AlanPerlisQuotesOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AlanPerlisQuotesMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AlanPerlisQuotesSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AlanPerlisQuotesMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AlanPerlisQuotesResponse([]),
            'result' => new AlanPerlisQuotesResult([]),
        ]);
        [, $err] = AlanPerlisQuotesMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AlanPerlisQuotesSpec([]),
            'response' => null,
            'result' => new AlanPerlisQuotesResult([]),
        ]);
        [, $err] = AlanPerlisQuotesMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AlanPerlisQuotesSpec(['step' => 's']),
            'response' => new AlanPerlisQuotesResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AlanPerlisQuotesResult([]),
        ]);
        [, $err] = AlanPerlisQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AlanPerlisQuotesSpec(['step' => 's']),
            'response' => new AlanPerlisQuotesResponse(self::resp(200, ['v' => 1])),
            'result' => new AlanPerlisQuotesResult([]),
        ]);
        [, $err] = AlanPerlisQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AlanPerlisQuotesSpec(['step' => 's']),
            'response' => new AlanPerlisQuotesResponse(self::resp(200, ['v' => 2])),
            'result' => new AlanPerlisQuotesResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AlanPerlisQuotesMakeResponse::call($ctx);
    ...
    {
        $preset = new AlanPerlisQuotesResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AlanPerlisQuotesSpec([]),
            'response' => new AlanPerlisQuotesResponse([]),
            'result' => new AlanPerlisQuotesResult([]),
        ...
        [$got, $err] = AlanPerlisQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AlanPerlisQuotesResult([])]);
        [, $err] = AlanPerlisQuotesMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AlanPerlisQuotesSpec([]), 'result' => null]);
        [, $err] = AlanPerlisQuotesMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AlanPerlisQuotesSpec(['step' => 's']),
            'result' => new AlanPerlisQuotesResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AlanPerlisQuotesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AlanPerlisQuotesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AlanPerlisQuotesSpec(['step' => 's']),
            'result' => new AlanPerlisQuotesResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AlanPerlisQuotesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AlanPerlisQuotesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AlanPerlisQuotesResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AlanPerlisQuotesSpec([]),
            'result' => new AlanPerlisQuotesResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AlanPerlisQuotesMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AlanPerlisQuotesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AlanPerlisQuotesError('boom', 'boom');
        $utility->fetcher = function (AlanPerlisQuotesContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AlanPerlisQuotesSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AlanPerlisQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AlanPerlisQuotesContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AlanPerlisQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AlanPerlisQuotesResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        AlanPerlisQuotesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AlanPerlisQuotesContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AlanPerlisQuotesSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AlanPerlisQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AlanPerlisQuotesResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AlanPerlisQuotesSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AlanPerlisQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AlanPerlisQuotesMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AlanPerlisQuotesMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AlanPerlisQuotesResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AlanPerlisQuotesDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AlanPerlisQuotesResult(['ok' => false])]);
        $this->expectException(AlanPerlisQuotesError::class);
        AlanPerlisQuotesDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AlanPerlisQuotesResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AlanPerlisQuotesMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AlanPerlisQuotesMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AlanPerlisQuotesBaseFeature();
        $b = new AlanPerlisQuotesBaseFeature();
        AlanPerlisQuotesFeatureAdd::call($ctx, $a);
        AlanPerlisQuotesFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AlanPerlisQuotesBaseFeature
    {
        $f = new AlanPerlisQuotesBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AlanPerlisQuotesFeatureAdd::call($ctx, self::named_feature('a'));
        AlanPerlisQuotesFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        AlanPerlisQuotesFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        AlanPerlisQuotesFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        AlanPerlisQuotesFeatureAdd::call($ctx, $replace);
        ...
        AlanPerlisQuotesFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?AlanPerlisQuotesSpec $spec): AlanPerlisQuotesContext
    {
    ...
    private static function auth_bags(): AlanPerlisQuotesSpec
    {
        return new AlanPerlisQuotesSpec(['headers' => [], 'query' => []]);
    }
    ...
    private static function auth_probe(array $options): ?array
    {
        ...
        AlanPerlisQuotesPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AlanPerlisQuotesPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AlanPerlisQuotesResponse(['headers' => null]),
            'result' => new AlanPerlisQuotesResult([]),
        ]);
        AlanPerlisQuotesResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        AlanPerlisQuotesResultBody::call($ctx);
