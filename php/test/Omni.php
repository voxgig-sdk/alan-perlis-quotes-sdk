...
//    only - a typed AlanPerlisQuotesContext there would make every ctx
...
final class AlanPerlisQuotesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AlanPerlisQuotesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new AlanPerlisQuotesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new AlanPerlisQuotesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new AlanPerlisQuotesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new AlanPerlisQuotesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?AlanPerlisQuotesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AlanPerlisQuotesError(is_string($code) ? $code : '', $msg);
    }
