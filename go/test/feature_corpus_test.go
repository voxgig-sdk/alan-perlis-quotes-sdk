...
//
// NewAlanPerlisQuotesSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.AlanPerlisQuotesSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewAlanPerlisQuotesSDK(opts)
}
...
func fcCandidates(client *sdk.AlanPerlisQuotesSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.AlanPerlisQuotesSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.AlanPerlisQuotesSDK, name string) any {
