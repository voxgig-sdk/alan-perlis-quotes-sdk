...
import (
	...
	omni "github.com/voxgig-sdk/alan-perlis-quotes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AlanPerlisQuotesError); is {
				out := map[string]any{
					"name":    "AlanPerlisQuotesError",
...
func omniCtx(arg any, client *sdk.AlanPerlisQuotesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AlanPerlisQuotesSDK); is {
			client = live
