...
func TestQuoteEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ALAN_PERLIS_QUOTES_TEST_QUOTE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func quoteBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ALAN_PERLIS_QUOTES_TEST_QUOTE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ALAN_PERLIS_QUOTES_TEST_QUOTE_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["ALAN_PERLIS_QUOTES_TEST_LIVE"] == "TRUE" {
		mergedOpts := vs.Merge([]any{
			map[string]any{
			},
			extra,
		})
		client = sdk.NewAlanPerlisQuotesSDK(core.ToMapAny(mergedOpts))
	}
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["ALAN_PERLIS_QUOTES_TEST_EXPLAIN"] == "TRUE",
		live:          live,
