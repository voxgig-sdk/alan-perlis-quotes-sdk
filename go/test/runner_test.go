...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("ALAN_PERLIS_QUOTES_TEST_LIVE") == "TRUE" ||
		os.Getenv("ALAN_PERLIS_QUOTES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("ALAN_PERLIS_QUOTES_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.AlanPerlisQuotesSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.AlanPerlisQuotesSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.AlanPerlisQuotesError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.AlanPerlisQuotesSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.AlanPerlisQuotesError{Msg: msg, Code: code}
}
