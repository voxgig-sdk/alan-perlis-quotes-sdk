...
module AlanPerlisQuotesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AlanPerlisQuotesResponse.new({})
    ...
    if fetch_err
      ...
      response = AlanPerlisQuotesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AlanPerlisQuotesResponse.new(fetched)
    else
