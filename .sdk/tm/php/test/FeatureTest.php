...
// AlanPerlisQuotes SDK feature test
//
...
class FtCtrl extends AlanPerlisQuotesControl
{
...
class FtClient
{
    ...
    private AlanPerlisQuotesUtility $utility;

    public function __construct(AlanPerlisQuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): AlanPerlisQuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AlanPerlisQuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AlanPerlisQuotesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AlanPerlisQuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AlanPerlisQuotesConfig::make_feature($name);
    ...
    private static function buildUrl(AlanPerlisQuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AlanPerlisQuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AlanPerlisQuotesResponse($fetched);
        }
    ...
    private function fail(AlanPerlisQuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AlanPerlisQuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
