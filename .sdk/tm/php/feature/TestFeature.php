...
// AlanPerlisQuotes SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AlanPerlisQuotesTestFeature extends AlanPerlisQuotesBaseFeature
{
    ...
    public function init(AlanPerlisQuotesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AlanPerlisQuotesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AlanPerlisQuotesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AlanPerlisQuotesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AlanPerlisQuotesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AlanPerlisQuotesParam::call($ctx, $k);
