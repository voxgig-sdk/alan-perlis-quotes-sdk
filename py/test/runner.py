# AlanPerlisQuotes SDK test runner
...
class AlanPerlisQuotesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AlanPerlisQuotesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AlanPerlisQuotesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AlanPerlisQuotesTestRunner.getenv("ALAN_PERLIS_QUOTES_TEST_LIVE")
        override = AlanPerlisQuotesTestRunner.getenv("ALAN_PERLIS_QUOTES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AlanPerlisQuotesTestRunner.getenv(key)
        ...
        explain = AlanPerlisQuotesTestRunner.getenv("ALAN_PERLIS_QUOTES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ALAN_PERLIS_QUOTES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AlanPerlisQuotesTestRunner._test_control is not None:
            return AlanPerlisQuotesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AlanPerlisQuotesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AlanPerlisQuotesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AlanPerlisQuotesTestRunner.load_test_control()
...
def load_env_local():
    AlanPerlisQuotesTestRunner.load_env_local()


def env_override(m):
    return AlanPerlisQuotesTestRunner.env_override(m)


def entity_data(v):
    return AlanPerlisQuotesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AlanPerlisQuotesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AlanPerlisQuotesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AlanPerlisQuotesTestRunner.load_test_control()


def live_client_options():
    return AlanPerlisQuotesTestRunner.live_client_options()


def live_delay_ms():
    return AlanPerlisQuotesTestRunner.live_delay_ms()
