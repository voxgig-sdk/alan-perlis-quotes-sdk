...
from alanperlisquotes_sdk.core.helpers import get_ctx_prop, to_map
...
class AlanPerlisQuotesContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = AlanPerlisQuotesControl()
        ...
        if isinstance(sp, AlanPerlisQuotesSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, AlanPerlisQuotesResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, AlanPerlisQuotesResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return AlanPerlisQuotesOperation({})
    ...
    def make_error(self, code, msg):
        return AlanPerlisQuotesError(code, msg, self)
