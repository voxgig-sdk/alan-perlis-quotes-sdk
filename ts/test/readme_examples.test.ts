...
//     (\`new AlanPerlisQuotesSDK(...)\` / \`AlanPerlisQuotesSDK.test(...)\` become
//     \`AlanPerlisQuotesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AlanPerlisQuotesSDK'
...
// rewrite real client construction (\`new AlanPerlisQuotesSDK(...)\` and any
// \`AlanPerlisQuotesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AlanPerlisQuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AlanPerlisQuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
